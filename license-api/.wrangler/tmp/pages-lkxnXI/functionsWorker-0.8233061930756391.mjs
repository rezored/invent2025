...
// ../.wrangler/tmp/bundle-Asj0is/checked-fetch.js
...
var onRequest = /* @__PURE__ */ __name(async (context) => {
  ...
  const configuredSecret = env.ADMIN_SECRET || "default-secret-change-me";
  ...
  if (request.method === "POST") {
    try {
      ...
      const limit = body.quota_limit || (type === "PRO" ? 1e6 : 20);
      await env.DB.prepare("INSERT INTO licenses (key, quota_limit, type, domain, is_active) VALUES (?, ?, ?, ?, 1)").bind(key, limit, type, domain).run();
      return new Response(JSON.stringify({ success: true, key, type, domain }), {
        headers: { "Content-Type": "application/json" }
      });
    } catch (e) {
...
var onRequest2 = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    ...
    if (!body.api_key || body.api_key === "FREE") {
      const existing = await env.DB.prepare("SELECT * FROM licenses WHERE domain = ? AND type = 'FREE'").bind(body.site_url).first();
      if (existing) {
        license = existing;
        finalKey = existing.key;
      } else {
        finalKey = `FREE-${crypto.randomUUID().substring(0, 8)}`;
        await env.DB.prepare("INSERT INTO licenses (key, quota_limit, type, domain, is_active) VALUES (?, 20, 'FREE', ?, 1)").bind(finalKey, body.site_url).run();
        license = { key: finalKey, quota_limit: 20, is_active: 1, type: "FREE" };
      }
    } else {
      license = await env.DB.prepare("SELECT * FROM licenses WHERE key = ?").bind(body.api_key).first();
    ...
    const usageResult = await env.DB.prepare("SELECT SUM(items_processed) as total_usage FROM usage_logs WHERE license_key = ?").bind(license.key).first();
    ...
    await env.DB.prepare("INSERT INTO usage_logs (license_key, items_processed, ip_address) VALUES (?, ?, ?)").bind(license.key, requestedCount, clientIP).run();
...
// ../.wrangler/tmp/pages-lkxnXI/functionsRoutes-0.13632085025787988.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-Asj0is/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-Asj0is/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.8233061930756391.mjs.map
